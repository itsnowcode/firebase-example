...
class _MyHomePageState extends State<MyHomePage> {
  ...
  DocumentReference countRef =
      FirebaseFirestore.instance.collection('datas').doc('count');
  ...
  void _incrementCounter() {
    ...
    _counter++;
    countRef.set({'count': _counter});
  }

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: countRef.snapshots(),
      builder:
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Text('Loading', key: const Key('loading1'));
        }

        final data = snapshot.data.data();
        if (data == null) {
          countRef.set({'count': 0});
          return const Text('Loading', key: const Key('loading2'));
        }
